...
# ---------------------------------------------------------------------------
# Cycle-3 hardening: --samples validation + JudgeError on API failure
# ---------------------------------------------------------------------------


class TestSamplesValidation:
    def test_main_rejects_zero_samples(self, capsys):
        from evals.run_eval import main
        rc = main(["--judge", "fake", "--samples", "0", "--fixture", "pr-8"])
        assert rc == 1
        err = capsys.readouterr().err
        assert "--samples must be >= 1" in err

    def test_main_rejects_negative_samples(self, capsys):
        from evals.run_eval import main
        rc = main(["--judge", "fake", "--samples", "-1", "--fixture", "pr-8"])
        assert rc == 1
        err = capsys.readouterr().err
        assert ">= 1" in err

    def test_main_accepts_samples_one(self):
        from evals.run_eval import main
        rc = main(["--judge", "fake", "--samples", "1", "--fixture", "pr-8"])
        assert rc == 0


class TestJudgeErrorCaughtInMain:
    def test_main_catches_judge_error_and_exits_2(self, capsys, monkeypatch):
        """Simulate a JudgeError surfacing from evaluate_fixture during main()."""
        from evals import run_eval as runner

        def raising(*_a, **_kw):
            raise JudgeError("simulated API failure")

        monkeypatch.setattr(runner, "evaluate_fixture", raising)
        rc = runner.main(["--judge", "fake", "--fixture", "pr-8"])
        assert rc == 2
        err = capsys.readouterr().err
