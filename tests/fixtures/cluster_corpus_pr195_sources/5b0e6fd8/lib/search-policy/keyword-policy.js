...
const SOFTWARE_ROLE_NOUN_PATTERN =
  /\b(engineer|engineering|architect|scientist|team lead|tech lead|group lead)\b/i;
...
function namesDevelopmentLanguage(criterion) {
  return [criterion.term, ...(criterion.anyOf ?? [])].some((member) =>
    DEVELOPMENT_FAMILIES.has(technologyFamily(member) ?? ""),
  );
}
...
export function ensureRequiredDevelopmentKeyword(criteria, title) {
  ...
  if (skillCriteria.some((criterion) => criterion.required && namesDevelopmentLanguage(criterion))) {
    return criteria;
  }
  const gate = skillCriteria.find(
    (criterion) => namesDevelopmentLanguage(criterion) && isGateEligibleKeywordTerm(criterion.term),
  );
...
function reservedGateIndex(criteria) {
...
function limitRequiredSourceGroups(criteria) {
  const gateIndex = reservedGateIndex(criteria);
